...
complete --command andiacoin-qt --no-files

# Extract options
function __fish_andiacoinqt_get_options
...
complete \
    --command andiacoin-qt \
    --arguments "(__fish_andiacoinqt_get_options)"
# Enable file completions only if the commandline now contains a `*.=` style option
complete -c andiacoin-qt \
...
complete \
    --command andiacoin-qt \
    --arguments "(__fish_andiacoinqt_get_options --nofiles)"
