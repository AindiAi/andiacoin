# Bootstrappable Andiacoin Core Builds

This directory contains the files necessary to perform bootstrappable Andiacoin
Core builds.
...
To build Andiacoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Andiacoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/andiacoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/andiacoin-core/guix.sigs).
