...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing andiacoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='andiacoin-cli', help='andiacoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Andiacoin address to which the faucet should send')
...
parser.add_argument('andiacoin_cli_args', nargs='*', help='Arguments to pass on to andiacoin-cli (default: -signet)')

args = parser.parse_args()

if args.andiacoin_cli_args == []:
    args.andiacoin_cli_args = ['-signet']


def andiacoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.andiacoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = andiacoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = andiacoin_cli(['getnewaddress', 'faucet', 'bech32'])
