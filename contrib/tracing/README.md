...
Tracing (USDT) support for Andiacoin Core on Linux using. For more information on
USDT support in Andiacoin Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `andiacoind` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `andiacoind` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[Andiacoin Core USDT documentation] on how to list available tracepoints in your
`andiacoind` binary.

[Andiacoin Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py ./src/andiacoind
```
...
Andiacoin P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py ./src/andiacoind
```
...
In a different terminal, starting Andiacoin Core in SigNet mode and with
re-indexing enabled.

```
$ ./src/andiacoind -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py ./src/andiacoind
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py ./src/andiacoind
```
