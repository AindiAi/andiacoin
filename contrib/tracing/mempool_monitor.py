...
""" Example logging Andiacoin Core mempool events using the mempool:added,
...
def main(andiacoind_path):
    andiacoind_with_usdts = USDT(path=str(andiacoind_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    andiacoind_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    andiacoind_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    andiacoind_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    andiacoind_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[andiacoind_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/andiacoind")
        exit(1)
