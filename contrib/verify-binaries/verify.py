...
"""Script for verifying Andiacoin Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from andiacoincore.org and andiacoin.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/andiacoin-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://andiacoincore.org"
HOST2 = "https://andiacoin.org"
VERSIONPREFIX = "andiacoin-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the Andiacoin community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"andiacoin_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by andiacoincore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the andiacoin release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes andiacoin.org lags behind andiacoincore.org.)')
    )
