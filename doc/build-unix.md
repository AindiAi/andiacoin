UNIX BUILD NOTES
====================
Some notes on how to build Andiacoin Core in Unix.
...
memory available when compiling Andiacoin Core. On systems with less, gcc can be
...
To build Andiacoin Core without wallet, see [*Disable-wallet mode*](#disable-wallet-mode)
...
If you want to build andiacoin-qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a andiacoin-qt executable will be
built by default.
...
...
to: /path/to/andiacoin/depends/x86_64-pc-linux-gnu
```
and configure using the following:
```bash
export BDB_PREFIX="/path/to/andiacoin/depends/x86_64-pc-linux-gnu"
...
Disable-wallet mode
--------------------
When the intention is to only run a P2P node, without a wallet, Andiacoin Core can
