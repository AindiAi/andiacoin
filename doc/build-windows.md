...
Below are some notes on how to build Andiacoin Core for Windows.

The options known to work for building Andiacoin Core on Windows are:
...
Note that for WSL the Andiacoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/andiacoin, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\andiacoin`, for example:

    make install DESTDIR=/mnt/c/workspace/andiacoin
