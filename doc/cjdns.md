# CJDNS support in Andiacoin Core

It is possible to run Andiacoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Andiacoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Andiacoin network and individual nodes.
...
before it will work with your Andiacoin Core node. This is described in steps
...
## Run Andiacoin Core with CJDNS

Once you are connected to the CJDNS network, the following Andiacoin Core
...
When enabled, this option tells Andiacoin Core that it is running in an
...
IPv6 local network. This helps Andiacoin Core perform better address management:
...
CJDNS support was added to Andiacoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `andiacoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/andiacoin/andiacoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Andiacoin Core

There are several ways to see your CJDNS address in Andiacoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `andiacoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `andiacoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `andiacoin-cli help getnodeaddresses` for details.
