# Fuzzing Andiacoin Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing Andiacoin Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):

```sh
$ git clone https://github.com/andiacoin/andiacoin
$ cd andiacoin/
$ ./autogen.sh
$ CC=clang CXX=clang++ ./configure --enable-fuzz --with-sanitizers=address,fuzzer,undefined
# macOS users: If you have problem with this step then make sure to read "macOS hints for
# libFuzzer" on https://github.com/andiacoin/andiacoin/blob/master/doc/fuzzing.md#macos-hints-for-libfuzzer
$ make
...
## Overview of Andiacoin Core fuzzing

[Google](https://github.com/google/fuzzing/) has a good overview of fuzzing in general, with contributions from key architects of some of the most-used fuzzers. [This paper](https://agroce.github.io/andiacoin_report.pdf) includes an external overview of the status of Andiacoin Core fuzzing, as of summer 2021.  [John Regehr](https://blog.regehr.org/archives/1687) provides good advice on writing code that assists fuzzers in finding bugs, which is useful for developers to keep in mind.

## Fuzzing harnesses and output

[`process_message`](https://github.com/andiacoin/andiacoin/blob/master/src/test/fuzz/process_message.cpp) is a fuzzing harness for the [`ProcessMessage(...)` function (`net_processing`)](https://github.com/andiacoin/andiacoin/blob/master/src/net_processing.cpp). The available fuzzing harnesses are found in [`src/test/fuzz/`](https://github.com/andiacoin/andiacoin/tree/master/src/test/fuzz).
...
It is possible to specify `andiacoind` arguments to the `fuzz` executable.
...
The project's collection of seed corpora is found in the [`andiacoin-core/qa-assets`](https://github.com/andiacoin-core/qa-assets) repo.

To fuzz `process_message` using the [`andiacoin-core/qa-assets`](https://github.com/andiacoin-core/qa-assets) seed corpus:

```sh
$ git clone https://github.com/andiacoin-core/qa-assets
...
Fuzzing on a harness compiled with `--with-sanitizers=address,fuzzer,undefined` is good for finding bugs. However, the very slow execution even under libFuzzer will limit the ability to find new coverage. A good approach is to perform occasional long runs without the additional bug-detectors (configure `--with-sanitizers=fuzzer`) and then merge new inputs into a corpus as described in the qa-assets repo (https://github.com/andiacoin-core/qa-assets/blob/main/.github/PULL_REQUEST_TEMPLATE.md).  Patience is useful; even with improved throughput, libFuzzer may need days and 10s of millions of executions to reach deep/hard targets.
...
If you find coverage increasing inputs when fuzzing you are highly encouraged to submit them for inclusion in the [`andiacoin-core/qa-assets`](https://github.com/andiacoin-core/qa-assets) repo.

Every single pull request submitted against the Andiacoin Core repo is automatically tested against all inputs in the [`andiacoin-core/qa-assets`](https://github.com/andiacoin-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make Andiacoin Core more robust.
...
# Fuzzing Andiacoin Core using afl++

## Quickstart guide

To quickly get started fuzzing Andiacoin Core using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
# Fuzzing Andiacoin Core using Honggfuzz

## Quickstart guide

To quickly get started fuzzing Andiacoin Core using [Honggfuzz](https://github.com/google/honggfuzz):
...
## Fuzzing the Andiacoin Core P2P layer using Honggfuzz NetDriver

Honggfuzz NetDriver allows for very easy fuzzing of TCP servers such as Andiacoin
Core without having to write any custom fuzzing harness. The `andiacoind` server
...
```sh
$ mkdir andiacoin-honggfuzz-p2p/
$ cd andiacoin-honggfuzz-p2p/
$ git clone https://github.com/andiacoin/andiacoin
$ cd andiacoin/
$ ./autogen.sh
$ git clone https://github.com/google/honggfuzz
...
EOF
$ make -C src/ andiacoind
$ mkdir -p inputs/
$ honggfuzz/honggfuzz --exit_upon_crash --quiet --timeout 4 -n 1 -Q \
      -E HFND_TCP_PORT=18444 -f inputs/ -- \
          src/andiacoind -regtest -discover=0 -dns=0 -dnsseed=0 -listenonion=0 \
                       -nodebuglogfile -bind=127.0.0.1:18444 -logthreadnames \
                       -debug
```

# Fuzzing Andiacoin Core using Eclipser (v1.x)

## Quickstart guide

To quickly get started fuzzing Andiacoin Core using [Eclipser v1.x](https://github.com/SoftSec-KAIST/Eclipser/tree/v1.x):
...
See [this proposed patch](https://github.com/andiacoin/andiacoin/pull/22472) for more information.
...
Andiacoin Core participates in Google's [OSS-Fuzz](https://github.com/google/oss-fuzz/tree/master/projects/andiacoin-core)
program, which includes a dashboard of [publicly disclosed vulnerabilities](https://bugs.chromium.org/p/oss-fuzz/issues/list?q=andiacoin-core).
...
because Andiacoin is a live P2P network, and not just standalone local software,
...
OSS-Fuzz also produces [a fuzzing coverage report](https://oss-fuzz.com/coverage-report/job/libfuzzer_asan_andiacoin-core/latest).
