# PSBT Howto for Andiacoin Core

Since Andiacoin Core 0.17, an RPC interface exists for Partially Signed Andiacoin
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/andiacoin/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Andiacoin transactions that are not fully signed
...
#### Multisig with multiple Andiacoin Core instances
...
Andiacoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `andiacoin-cli` is used.
...
Later, when *V* ADC has been received on *Amulti*, and Bob and Carol want to
