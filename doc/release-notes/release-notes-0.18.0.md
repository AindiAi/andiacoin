Andiacoin Core version 0.18.0 is now available from:

  <https://andiacoincore.org/bin/andiacoin-core-0.18.0/>
...
`/Applications/Andiacoin-Qt` (on Mac) or `andiacoind`/`andiacoin-qt` (on
Linux).
...
recommended to use Andiacoin Core on unsupported systems.

Andiacoin Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, Andiacoin Core does not yet change appearance when
...
`contrib/init/andiacoind.service`, it has been changed to use
`/var/lib/andiacoind` as the data directory instead of
`~andiacoin/.andiacoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/andiacoind` will
exist has enough space (check using `df -h /var/lib/andiacoind`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/andiacoin/andiacoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://andiacoincore.org/en/contact/) the Andiacoin Core
...
The systemd init file (`contrib/init/andiacoind.service`) has been changed
to use `/var/lib/andiacoind` as the data directory instead of
`~andiacoin/.andiacoin`. This change makes Andiacoin Core more consistent with
...
`datadir` option in `/etc/andiacoin/andiacoin.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/andiacoin/andiacoin.conf`.


Documentation
-------------

- A new short [document](https://github.com/andiacoin/andiacoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/andiacoin/andiacoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/andiacoin/andiacoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/andiacoin/andiacoin/blob/master/doc/andiacoin-conf.md)
  about the `andiacoin.conf` file describes how to use it to configure
  Andiacoin Core.

- A new document introduces Andiacoin Core's BIP174 [Partially-Signed
  Andiacoin Transactions
  (PSBT)](https://github.com/andiacoin/andiacoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/andiacoin/andiacoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Andiacoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Andiacoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/andiacoin/andiacoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/andiacoin/andiacoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Andiacoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Andiacoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/andiacoin/andiacoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/andiacoin/andiacoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart andiacoind with the `-deprecatedrpc=generate`
...
- A new `andiacoin-wallet` tool is now distributed alongside Andiacoin
...
This section describes planned changes to Andiacoin Core that may affect
other Andiacoin software and services.

- Since version 0.16.0, Andiacoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Andiacoin Core 0.20 (expected about
  a year after 0.18), Andiacoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Andiacoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Andiacoin Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Andiacoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Andiacoin Core
...
- A sub-project of Andiacoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Andiacoin Core.  See their [project
  page](https://github.com/andiacoin-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Andiacoin Core's own implementation, although entropy
  gathered by Andiacoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Andiacoin Core a
...
- On macOS, Andiacoin Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `andiacoin_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libandiacoin_server.a` (jnewbery)
...
- #13926 [Tools] andiacoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like andiacoind (MarcoFalke)
...
- #14521 qt, docs: Fix `andiacoin-qt -version` output formatting (hebasto)
...
- #11625 Add AndiacoinApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libandiacoin_server` (laanwj)
...
- #15154 Configure: andiacoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/andiacoin-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Andiacoin Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libandiacoin_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in andiacoin-util-test.py (isghe)
...
- #12775 Integration of property based testing into Andiacoin Core (Christewart)
...
- #12246 Bugfix: Only run andiacoin-tx tests when andiacoin-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_andiacoin` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_andiacoin` (MarcoFalke)
...
- #14186 andiacoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_andiacoin.conf` message on startup if conf file exists (leishman)
...
- #12255 Update andiacoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/andiacoin-conf.md (hebasto)
...
- #15354 Add missing `andiacoin-wallet` tool manpages (MarcoFalke)
...
- #15765 correct andiacoinconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support andiacoin folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/andiacoin/).
