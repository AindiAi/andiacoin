...
Andiacoin Core version 0.19.1 is now available from:

  <https://andiacoincore.org/bin/andiacoin-core-0.19.1/>
...
From Andiacoin Core 0.17.0 onwards, macOS versions earlier than 10.10 are no
longer supported, as Andiacoin Core is now built using Qt 5.9.x which requires
macOS 10.10+. Additionally, Andiacoin Core does not yet change appearance when
...
- #17488 fix "andiacoind already running" warnings on macOS (fanquake)
