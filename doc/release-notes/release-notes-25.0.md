...
Andiacoin Core version 25.0 is now available from:

  <https://andiacoincore.org/bin/andiacoin-core-25.0/>
...
- All JSON-RPC methods accept a new [named
  parameter](https://github.com/andiacoin/andiacoin/blob/master/doc/JSON-RPC-interface.md#parameter-passing) called `args` that can
  contain positional parameter values. This is a convenience to allow some
  parameter values to be passed by name without having to name every value. The
  python test framework and `andiacoin-cli` tool both take advantage of this, so
  for example:

```sh
andiacoin-cli -named createwallet wallet_name=mywallet load_on_startup=1
```

Can now be shortened to:

```sh
andiacoin-cli -named createwallet mywallet load_on_startup=1
```
...
dbcache, Andiacoin Core will now return an error at startup. (#25574)
...
before Andiacoin Core has begun its shutdown sequence. (#23395)
...
- In the `createwallet`, `loadwallet`, `unloadwallet`, and `restorewallet` RPCs, the
  ...
  deprecation period by launching andiacoind with the configuration option
...
- The binary verification script has been updated. In previous releases it
  ...
  examples, see:
  https://github.com/andiacoin/andiacoin/blob/master/contrib/verify-binaries/README.md
  (#27358)
