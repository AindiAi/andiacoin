...
Andiacoin Core version 26.0 is now available from:

  <https://andiacoincore.org/bin/andiacoin-core-26.0/>
...
using the Linux kernel, macOS 11.0+, and Windows 7 and newer.  Andiacoin
...
- Experimental support for the v2 transport protocol defined in
  [BIP324](https://github.com/andiacoin/bips/blob/master/bip-0324.mediawiki) was added.
...
- [Miniscript](https://andiacoin.sipa.be/miniscript/) expressions can now be used in Taproot descriptors for all RPCs working with descriptors. (#27255)

- `finalizepsbt` is now able to finalize a PSBT with inputs spending [Miniscript](https://andiacoin.sipa.be/miniscript/)-compatible Taproot leaves. (#27255)
...
- `andiacoind` and `andiacoin-qt` will now raise an error on startup
 if a datadir that is being used contains a andiacoin.conf file that
 will be ignored, which can happen when a datadir= line is used in
 a andiacoin.conf file. The error message is just a diagnostic intended
 ...
 the andiacoin.conf contained in it. (#27302)
...
- A new `andiacoinconsensus_verify_script_with_spent_outputs` function is available in libconsensus which optionally accepts the spent outputs of the transaction being verified.
- A new `andiacoinconsensus_SCRIPT_FLAGS_VERIFY_TAPROOT` flag is available in libconsensus that will verify scripts with the Taproot spending rules.
...
- Wallet loading has changed in this release. Wallets with some corrupted records that could be
  ...
  load the wallet in a previous version of Andiacoin Core and import the data into a new wallet.
...
```sh
src/andiacoin-cli -named bumpfee txid fee_rate=100
```

instead of

```sh
src/andiacoin-cli -named bumpfee txid options='{"fee_rate": 100}'
```
...
- It's now possible to use [Miniscript](https://andiacoin.sipa.be/miniscript/) inside Taproot leaves for descriptor wallets. (#27255)
...
- The usage of hybrid public keys in output descriptors has been removed. Hybrid
  ...
  (as specified in BIP380 and documented in doc/descriptors.md). Andiacoin Core would
...
- Bash completion files have been renamed from `andiacoin*.bash-completion` to
  `andiacoin*.bash`. This means completions can be automatically loaded on demand
