# User-space, Statically Defined Tracing (USDT) for Andiacoin Core

Andiacoin Core includes statically defined tracepoints to allow for more
...
**Note**: Andiacoin Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Andiacoin Core
...
Multiple tools can list the available tracepoints in a `andiacoind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Andiacoin Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/andiacoind
stap net        outbound_message 0x0000000000107c05 /src/andiacoind
stap validation block_connected  0x00000000002fb10c /src/andiacoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Andiacoin Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/andiacoind | grep NT_STAPSDT -A 4 -B 2
