...
std::optional<ConfigError> InitConfig(ArgsManager& args, SettingsAbortFn settings_abort_fn)
{
    try {
        ...
        // andiacoin.conf file just ignores the other file.)
        const fs::path orig_datadir_path{args.GetDataDirBase()};
        const fs::path orig_config_path{AbsPathForConfigVal(args, args.GetPathArg("-conf", ANDIACOIN_CONF_FILENAME), /*net_specific=*/false)};
        ...
        if (!fs::exists(base_path)) {
            ...
            // in https://github.com/andiacoin/andiacoin/issues/16220 about ways to
        ...
        // Show an error or warning if there is a andiacoin.conf file in the
        // datadir that is being ignored.
        const fs::path base_config_path = base_path / ANDIACOIN_CONF_FILENAME;
        if (fs::exists(base_config_path) && !fs::equivalent(orig_config_path, base_config_path)) {
            ...
            const std::string error = strprintf(
                ...
                fs::quoted(ANDIACOIN_CONF_FILENAME),
