 ...
 */
static const char* ANDIACOIN_PID_FILENAME = "andiacoind.pid";
/**
...
{
    return AbsPathForConfigVal(args, args.GetPathArg("-pid", ANDIACOIN_PID_FILENAME));
}
...
void SetupServerArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-conf=<file>", strprintf("Specify path to read-only configuration file. Relative paths will be prefixed by datadir location (only useable from command line, not configuration file) (default: %s)", ANDIACOIN_CONF_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    argsman.AddArg("-allowignoredconf", strprintf("For backwards compatibility, treat an unused %s file in the datadir as a warning, not an error.", ANDIACOIN_CONF_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    argsman.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", ANDIACOIN_PID_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    argsman.AddArg("-settings=<file>", strprintf("Specify path to dynamic settings data file. Can be disabled with -nosettings. File is written at runtime and not meant to be edited by users (use %s instead for custom settings). Relative paths will be prefixed by datadir location. (default: %s)", ANDIACOIN_CONF_FILENAME, ANDIACOIN_SETTINGS_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
#if HAVE_SYSTEM
    ...
    // https://github.com/andiacoin/andiacoin/pull/23542 have become widespread.
...
{
    // Make sure only a single Andiacoin process is using the data directory.
...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if (args.IsArgSet("-datadir") && !args.GetPathArg("-datadir").is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if andiacoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if andiacoin is started while in a temporary directory.\n",
