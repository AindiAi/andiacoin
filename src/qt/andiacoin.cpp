...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<AndiacoinUnit>("AndiacoinUnit");
#else
    qRegisterMetaType<AndiacoinUnit>("AndiacoinUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. andiacoin_de.qm (shortcut "de" needs to be defined in andiacoin.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. andiacoin_de_DE.qm (shortcut "de_DE" needs to be defined in andiacoin.qrc)
...
static const char* qt_argv = "andiacoin-qt";

AndiacoinApplication::AndiacoinApplication()
...
void AndiacoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the AndiacoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", AndiacoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void AndiacoinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool AndiacoinApplication::createOptionsModel(bool resetSettings)
{
...
void AndiacoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new AndiacoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &AndiacoinGUI::quitRequested, this, &AndiacoinApplication::requestShutdown);
...
void AndiacoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void AndiacoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool AndiacoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void AndiacoinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &AndiacoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &AndiacoinApplication::handleRunawayException);
    connect(this, &AndiacoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &AndiacoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void AndiacoinApplication::parameterSetup()
{
...
void AndiacoinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void AndiacoinApplication::requestInitialize()
{
...
void AndiacoinApplication::requestShutdown()
{
...
void AndiacoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // andiacoin: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &AndiacoinGUI::handlePaymentRequest);
            connect(window, &AndiacoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void AndiacoinApplication::handleRunawayException(const QString &message)
{
...
void AndiacoinApplication::handleNonFatalException(const QString& message)
{
...
WId AndiacoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool AndiacoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", AndiacoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(andiacoin);
    Q_INIT_RESOURCE(andiacoin_locale);
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(ANDIACOIN_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see andiacoin-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, PACKAGE_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see andiacoin-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse andiacoin.conf, determine network, switch to network specific
    ...
    // andiacoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
