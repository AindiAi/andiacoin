// Copyright (c) 2011-2022 The Andiacoin Core developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#if defined(HAVE_CONFIG_H)
#include <config/andiacoin-config.h>
#endif

#include <qt/andiacoingui.h>
...
const std::string AndiacoinGUI::DEFAULT_UIPLATFORM =
...
AndiacoinGUI::AndiacoinGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &AndiacoinGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &AndiacoinGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &AndiacoinGUI::showModalOverlay);
...
void AndiacoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Andiacoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and andiacoin: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &AndiacoinGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &AndiacoinGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &AndiacoinGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Andiacoin addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Andiacoin addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Andiacoin Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Andiacoin Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a andiacoin: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Andiacoin command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &AndiacoinGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &AndiacoinGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &AndiacoinGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &AndiacoinGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &AndiacoinGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &AndiacoinGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &AndiacoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &AndiacoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &AndiacoinGUI::createWallet);
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_migrate_wallet_action, &QAction::triggered, [this] {
            auto activity = new MigrateWalletActivity(m_wallet_controller, this);
            connect(activity, &MigrateWalletActivity::migrated, this, &AndiacoinGUI::setCurrentWallet);
            activity->migrate(walletFrame->currentWalletModel());
        });
        connect(m_mask_values_action, &QAction::toggled, this, &AndiacoinGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &AndiacoinGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &AndiacoinGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &AndiacoinGUI::showDebugWindow);
}

void AndiacoinGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void AndiacoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &AndiacoinGUI::setCurrentWalletBySelectorIndex);
...
void AndiacoinGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &AndiacoinGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &AndiacoinGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &AndiacoinGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &AndiacoinGUI::showProgress);
...
#ifdef ENABLE_WALLET
void AndiacoinGUI::enableHistoryAction(bool privacy)
{
...
void AndiacoinGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &AndiacoinGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &AndiacoinGUI::removeWallet);
...
WalletController* AndiacoinGUI::getWalletController()
{
    return m_wallet_controller;
}

void AndiacoinGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &AndiacoinGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &AndiacoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &AndiacoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &AndiacoinGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &AndiacoinGUI::incomingTransaction);
    connect(this, &AndiacoinGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void AndiacoinGUI::removeWallet(WalletModel* walletModel)
{
...
void AndiacoinGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void AndiacoinGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void AndiacoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void AndiacoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void AndiacoinGUI::createTrayIcon()
{
...
void AndiacoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &AndiacoinGUI::toggleHidden);
...
void AndiacoinGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void AndiacoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void AndiacoinGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void AndiacoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void AndiacoinGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void AndiacoinGUI::openClicked()
{
...
void AndiacoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void AndiacoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void AndiacoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void AndiacoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void AndiacoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void AndiacoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void AndiacoinGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void AndiacoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Andiacoin network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void AndiacoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void AndiacoinGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void AndiacoinGUI::updateHeadersSyncProgressLabel()
{
...
void AndiacoinGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void AndiacoinGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &AndiacoinGUI::quitRequested);
...
void AndiacoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void AndiacoinGUI::createWallet()
{
#ifdef ENABLE_WALLET
#ifndef USE_SQLITE
    ...
    connect(activity, &CreateWalletActivity::created, this, &AndiacoinGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void AndiacoinGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void AndiacoinGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &AndiacoinGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &AndiacoinGUI::show);
                e->ignore();
            }
        }
    }
...
void AndiacoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void AndiacoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void AndiacoinGUI::incomingTransaction(const QString& date, AndiacoinUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(AndiacoinUnits::formatWithUnit(unit, amount, true));
...
void AndiacoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void AndiacoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool AndiacoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool AndiacoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void AndiacoinGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void AndiacoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void AndiacoinGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void AndiacoinGUI::updateProxyIcon()
{
    std::string ip_port;
...
void AndiacoinGUI::updateWindowTitle()
{
...
void AndiacoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void AndiacoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void AndiacoinGUI::detectShutdown()
{
...
void AndiacoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void AndiacoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(AndiacoinGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = AndiacoinGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void AndiacoinGUI::subscribeToCoreSignals()
{
...
void AndiacoinGUI::unsubscribeFromCoreSignals()
{
...
bool AndiacoinGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<AndiacoinUnit> units = AndiacoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const AndiacoinUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, AndiacoinUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const AndiacoinUnit u : AndiacoinUnits::availableUnits()) {
        menu->addAction(AndiacoinUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(AndiacoinUnit newUnits)
{
    setText(AndiacoinUnits::longName(newUnits));
}
