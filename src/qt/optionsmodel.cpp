...
/** Call node.updateRwSetting() with Andiacoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Andiacoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/andiacoin/andiacoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because andiacoin 22.x will not try to read these.
...
//! Convert enabled/size values to andiacoin -prune setting.
...
//! Get pruning enabled value to show in GUI from andiacoin -prune setting.
...
//! Get pruning size value to show in GUI from andiacoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayAndiacoinUnit")) {
        settings.setValue("DisplayAndiacoinUnit", QVariant::fromValue(AndiacoinUnit::ADC));
    }
    QVariant unit = settings.value("DisplayAndiacoinUnit");
    if (unit.canConvert<AndiacoinUnit>()) {
        m_display_andiacoin_unit = unit.value<AndiacoinUnit>();
    } else {
        m_display_andiacoin_unit = AndiacoinUnit::ADC;
        settings.setValue("DisplayAndiacoinUnit", QVariant::fromValue(m_display_andiacoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if andiacoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_andiacoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<AndiacoinUnit>() == m_display_andiacoin_unit) return;
    m_display_andiacoin_unit = new_unit.value<AndiacoinUnit>();
    QSettings settings;
    settings.setValue("DisplayAndiacoinUnit", QVariant::fromValue(m_display_andiacoin_unit));
    Q_EMIT displayUnitChanged(m_display_andiacoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/andiacoin/andiacoin/pull/8273
    ...
    // (https://github.com/andiacoin-core/gui/issues/567).
