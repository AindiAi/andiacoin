This directory contains integration tests that test andiacoind and its
...
andiacoind and andiacoin-qt by interacting with them through the RPC and P2P
interfaces.
- [util](/test/util) which tests the utilities (andiacoin-util, andiacoin-tx, ...).
...
Before tests can be run locally, Andiacoin Core must be built.  See the [building instructions](/doc#building) for help.
...
The P2P and RPC ports used by the andiacoind nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another andiacoind
process running on the system (perhaps from a previous test which hasn't successfully
killed all its andiacoind nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other andiacoind processes are running.

On linux, the test framework will warn if there is another
andiacoind process running when the tests are started.

If there are zombie andiacoind processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
andiacoind processes running on the system, so should not be used if any non-test
andiacoind processes are being run.**
...
sure andiacoind processes are stopped as above):
...
- when run by [our CI (Continuous Integration)](/ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` and andiacoind `debug.log`s will all be dumped
...
`test_framework.log` and andiacoind `debug.log`s can be combined into a single
...
call methods that interact with the andiacoind nodes-under-test.

If further introspection of the andiacoind instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/andiacoind.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/andiacoind <pid>
```
