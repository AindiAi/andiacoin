#!/usr/bin/env python3
# Copyright (c) 2017-2022 The Andiacoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.
"""Class for andiacoind node under test"""
...
ANDIACOIND_PROC_WAIT_TIMEOUT = 60
...
class TestNode():
    """A class for representing a andiacoind node under test.
    ...
    def __init__(self, i, datadir_path, *, chain, rpchost, timewait, timeout_factor, andiacoind, andiacoin_cli, coverage_dir, cwd, extra_conf=None, extra_args=None, use_cli=False, start_perf=False, use_valgrind=False, version=None, descriptors=False, v2transport=False):
        """
        Kwargs:
        ...
        # Configuration for logging is set as command-line args rather than in the andiacoin.conf file.
        # This means that starting a andiacoind using the temp dir to debug a failed test won't
        # spam debug.log.
        self.args = [
            self.binary,
        ...
        self.cli = TestNodeCLI(andiacoin_cli, self.datadir_path)
    ...
    def __del__(self):
        # Ensure that we don't leave any andiacoind processes lying around after
        # the test ends
    ...
    def start(self, extra_args=None, *, cwd=None, stdout=None, stderr=None, env=None, **kwargs):
        ...
        # Add a new stdout and stderr file each time andiacoind is started
        if stderr is None:
        ...
        # unclean shutdown), it will get overwritten anyway by andiacoind, and
        ...
        self.running = True
        self.log.debug("andiacoind started, waiting for RPC to come up")

        if self.start_perf:
            self._start_perf()

    def wait_for_rpc_connection(self):
        """Sets up an RPC connection to the andiacoind process. Returns False if unable to connect."""
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                ...
                raise FailedToStartError(self._node_msg(
                    f'andiacoind exited with status {self.process.returncode} during initialization. {str_error}'))
            try:
                rpc = get_rpc_proxy(
            ...
            except ValueError as e:  # cookie file not found and no rpcuser or rpcpassword; andiacoind is still starting
                if "No RPC credentials" not in str(e):
                    raise
            time.sleep(1.0 / poll_per_s)
        self._raise_assertion_error("Unable to connect to andiacoind after {}s".format(self.rpc_timeout))
    ...
    def wait_for_cookie_credentials(self):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            try:
                ...
                return
            except ValueError:  # cookie file not found and no rpcuser or rpcpassword; andiacoind is still starting
    ...
    def wait_until_stopped(self, *, timeout=ANDIACOIND_PROC_WAIT_TIMEOUT, expect_error=False, **kwargs):
    ...
    def _start_perf(self, profile_name=None):
        ...
        if not test_success('readelf -S {} | grep .debug_str'.format(shlex.quote(self.binary))):
            self.log.warning(
                "perf output won't be very useful without debug symbols compiled into andiacoind")
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, match=ErrorMatch.FULL_TEXT, *args, **kwargs):
        ...
        extra_args: extra arguments to pass through to andiacoind
        expected_msg: regex that stderr should match when andiacoind fails

        Will throw if andiacoind starts without an error.
        Will throw if an expected_msg is provided and it does not match andiacoind's stdout."""
        ...
        with tempfile.NamedTemporaryFile(dir=self.stderr_dir, delete=False) as log_stderr, \
            ...
            try:
                ...
                self.log.debug(self._node_msg(f'andiacoind exited with status {ret} during initialization'))
                ...
                self.running = False
                self.process = None
                assert_msg = f'andiacoind should have exited within {self.rpc_timeout}s '
...
class TestNodeCLI():
    """Interface to andiacoin-cli for an individual node"""
    def __init__(self, binary, datadir):
        self.options = []
        self.binary = binary
        self.datadir = datadir
        self.input = None
        self.log = logging.getLogger('TestFramework.andiacoincli')

    def __call__(self, *options, input=None):
        # TestNodeCLI is callable with andiacoin-cli command-line options
    ...
    def send_cli(self, clicommand=None, *args, **kwargs):
        """Run andiacoin-cli command. Deserializes returned string as python object."""
        ...
        self.log.debug("Running andiacoin-cli {}".format(p_args[2:]))
