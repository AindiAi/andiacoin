...
def assert_fee_amount(fee, tx_size, feerate_ADC_kvB):
    """Assert the fee is in range."""
    assert isinstance(tx_size, int)
    target_fee = get_fee(tx_size, feerate_ADC_kvB)
    if fee < target_fee:
        raise AssertionError("Fee of %s ADC too low! (Should be %s ADC)" % (str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    high_fee = get_fee(tx_size + 2, feerate_ADC_kvB)
    if fee > high_fee:
        raise AssertionError("Fee of %s ADC too high! (Should be %s ADC)" % (str(fee), str(target_fee)))
...
def check_json_precision():
    """Make sure json library being used does not lose precision converting ADC values"""
...
def get_fee(tx_size, feerate_btc_kvb):
    """Calculate the fee in ADC given a feerate is ADC/kvB. Reflects CFeeRate::GetFee"""
    feerate_sat_kvb = int(feerate_btc_kvb * Decimal(1e8)) # Fee in sat/kvb as an int to avoid float precision errors
    target_fee_sat = ceildiv(feerate_sat_kvb * tx_size, 1000) # Round calculated fee up to nearest sat
    return target_fee_sat / Decimal(1e8) # Return result in  ADC
...
def wait_until_helper_internal(predicate, *, attempts=float('inf'), timeout=float('inf'), lock=None, timeout_factor=1.0):
    ...
    from `AndiacoinTestFramework` or `P2PInterface` class ensures the timeout is
...
def initialize_datadir(dirname, n, chain, disable_autoconnect=True):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    write_config(os.path.join(datadir, "andiacoin.conf"), n=n, chain=chain, disable_autoconnect=disable_autoconnect)
...
def get_temp_default_datadir(temp_dir: pathlib.Path) -> tuple[dict, pathlib.Path]:
    ...
    if platform.system() == "Windows":
        ...
        datadir = temp_dir / "Andiacoin"
    else:
        env = dict(HOME=str(temp_dir))
        if platform.system() == "Darwin":
            datadir = temp_dir / "Library/Application Support/Andiacoin"
        else:
            datadir = temp_dir / ".andiacoin"
    return env, datadir
...
def append_config(datadir, options):
    with open(os.path.join(datadir, "andiacoin.conf"), 'a', encoding='utf8') as f:
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "andiacoin.conf")):
        with open(os.path.join(datadir, "andiacoin.conf"), 'r', encoding='utf8') as f:
            for line in f:
