#!/usr/bin/env python3
# Copyright (c) 2018-2022 The Andiacoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.
"""Test andiacoin-wallet."""
...
from test_framework.test_framework import AndiacoinTestFramework
...
class ToolWalletTest(AndiacoinTestFramework):
    ...
    def andiacoin_wallet_process(self, *args):
        default_args = ['-datadir={}'.format(self.nodes[0].datadir_path), '-chain=%s' % self.chain]
        if not self.options.descriptors and 'create' in args:
            default_args.append('-legacy')

        return subprocess.Popen([self.options.andiacoinwallet] + default_args + list(args), stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)

    def assert_raises_tool_error(self, error, *args):
        p = self.andiacoin_wallet_process(*args)
    ...
    def write_dump(self, dump, filename, magic=None, skip_checksum=False):
        if magic is None:
            magic = "ANDIACOIN_CORE_WALLET_DUMP"
    ...
    def do_tool_createfromdump(self, wallet_name, dumpfile, file_format=None):
        ...
        self.assert_tool_output("The dumpfile may contain private keys. To ensure the safety of your Andiacoin, do not share the dumpfile.\n", '-wallet={}'.format(wallet_name), '-dumpfile={}'.format(rt_dumppath), 'dump')
    ...
    def test_invalid_tool_commands_and_args(self):
        ...
        # `andiacoin-wallet help` raises an error. Use `andiacoin-wallet -help`.
        ...
        self.assert_raises_tool_error('No method provided. Run `andiacoin-wallet -help` for valid methods.')
    ...
    def test_salvage(self):
        # TODO: Check salvage actually salvages and doesn't break things. https://github.com/andiacoin/andiacoin/issues/7463
    ...
    def test_dump_createfromdump(self):
        ...
        assert_equal(dump_data['ANDIACOIN_CORE_WALLET_DUMP'], '1')
        ...
        dump_data["ANDIACOIN_CORE_WALLET_DUMP"] = "0"
        self.write_dump(dump_data, bad_ver_wallet_dump)
        self.assert_raises_tool_error('Error: Dumpfile version is not supported. This version of andiacoin-wallet only supports version 1 dumpfiles. Got dumpfile with version 0', '-wallet=badload', '-dumpfile={}'.format(bad_ver_wallet_dump), 'createfromdump')
        assert not (self.nodes[0].wallets_path / "badload").is_dir()
        bad_ver_wallet_dump = self.nodes[0].datadir_path / "wallet-bad_ver2.dump"
        dump_data["ANDIACOIN_CORE_WALLET_DUMP"] = "2"
        self.write_dump(dump_data, bad_ver_wallet_dump)
        self.assert_raises_tool_error('Error: Dumpfile version is not supported. This version of andiacoin-wallet only supports version 1 dumpfiles. Got dumpfile with version 2', '-wallet=badload', '-dumpfile={}'.format(bad_ver_wallet_dump), 'createfromdump')
        assert not (self.nodes[0].wallets_path / "badload").is_dir()
        bad_magic_wallet_dump = self.nodes[0].datadir_path / "wallet-bad_magic.dump"
        del dump_data["ANDIACOIN_CORE_WALLET_DUMP"]
        dump_data["not_the_right_magic"] = "1"
        self.write_dump(dump_data, bad_magic_wallet_dump, "not_the_right_magic")
        self.assert_raises_tool_error('Error: Dumpfile identifier record is incorrect. Got "not_the_right_magic", expected "ANDIACOIN_CORE_WALLET_DUMP".', '-wallet=badload', '-dumpfile={}'.format(bad_magic_wallet_dump), 'createfromdump')
        assert not (self.nodes[0].wallets_path / "badload").is_dir()
